...
- [ ] P2. IPC contract checklist confirmed with real runtime (events + payload schemas + semantics)
  ...
  - [ ] P2.4 Signature request payload schema (confirm single vs batch)
    ...
    - Evidence (legacy): mpay wallet-side receiver reads `signaturedata` from **query first**, then falls back to `event.request.text()` (POST body). Meanwhile, mpay `externalFetch` sends `signaturedata` primarily via **POST body** (query-based `signaturedata` is commented out). KeyApp Phase B should implement **query first + body fallback** for maximum compatibility.
