...
import { scanValidators, getValidatorForChain } from '../scanner-validators'
import { parseQRContent } from '@/lib/qr-parser'

const meta: Meta = {
  title: 'Sheets/ScannerJob',
  parameters: {
    layout: 'centered',
  },
}
...
/** 验证器测试演示 */
export const ValidatorDemo: StoryObj = {
  render: () => {
    ...
    return (
      <div className="w-96 space-y-4 rounded-lg border p-6">
        ...
        <div className={`rounded p-3 ${result === true ? 'bg-green-100' : 'bg-red-100'}`}>
          {result === true ? (
            <p className="font-medium text-green-800">✓ 验证通过</p>
          ) : (
            <p className="font-medium text-red-800">✗ {result}</p>
          )}
        </div>
      </div>
    )
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {testCases.map((tc) => {
              ...
              return (
                <tr key={tc.label} className="border-b">
...
/** UI 模拟预览 */
export const UIPreview: StoryObj = {
  render: () => {
    const [message, setMessage] = useState<{ type: 'error' | 'success'; text: string } | null>(null)
    
    return (
      <div className="relative h-[600px] w-[375px] overflow-hidden rounded-2xl bg-black">
        {/* Handle */}
        ...
        {/* Camera View */}
        <div className="relative flex-1 bg-gray-900" style={{ height: 400 }}>
          {/* Simulated camera */}
          ...
          <div className="absolute inset-x-0 bottom-24 px-4 text-center">
            {message ? (
              <div className={`inline-block rounded-lg px-4 py-2 ${
                message.type === 'error' ? 'bg-red-500/80' : 'bg-green-500/80'
              } text-white`}>
                {message.text}
              </div>
            ) : (
              <p className="text-white/80">请提供 Ethereum 链的地址二维码</p>
            )}
          </div>
        </div>
        ...
        <div className="absolute bottom-4 left-4 right-4 flex gap-2">
          <button
            onClick={() => setMessage({ type: 'success', text: '二维码已扫描' })}
            className="flex-1 rounded bg-green-500 py-2 text-sm text-white"
          >
            模拟成功
          </button>
          <button
            onClick={() => setMessage({ type: 'error', text: '不是有效的以太坊地址' })}
          ...
          <button
            ...
            className="flex-1 rounded bg-gray-500 py-2 text-sm text-white"
          >
            重置
          </button>
        </div>
