 ...
 * 验证密码是否正确（尝试解密）
 */
export async function verifyPassword(encrypted: EncryptedData, password: string): Promise<boolean> {
  try {
 ...
 * 用于从助记词派生的密钥直接加密
 */
export async function encryptWithRawKey(plaintext: string, rawKey: Uint8Array): Promise<EncryptedData> {
  const encoder = new TextEncoder();
  const data = encoder.encode(plaintext);
 ...
 * 使用原始密钥解密
 */
export async function decryptWithRawKey(encrypted: EncryptedData, rawKey: Uint8Array): Promise<string> {
  const iv = base64ToUint8Array(encrypted.iv);
  const ciphertext = base64ToUint8Array(encrypted.ciphertext);
