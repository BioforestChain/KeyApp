/**
 * Etherscan V1 API Provider - Effect TS Version (深度重构)
 *
...
export class EtherscanV1ProviderEffect extends EvmIdentityMixin(EvmTransactionMixin(EtherscanBase)) implements ApiProvider {
  ...
  constructor(entry: ParsedApiEntry, chainId: string) {
    ...
    this.transactionHistory = createStreamInstanceFromSource<TxHistoryParams, TransactionsOutput>(
      `etherscan.${chainId}.transactionHistory`,
    ...
    this.nativeBalance = createStreamInstanceFromSource<AddressParams, BalanceOutput>(
      `etherscan.${chainId}.nativeBalance`,
      ...
      const createPromise = Effect.runPromise(
        Effect.gen(function* () {
          ...
          const source = yield* createPollingSource({
            name: `etherscan.${provider.chainId}.txHistory.${cacheKey}`,
            fetch: fetchEffect,
      ...
      const createPromise = Effect.runPromise(
        Effect.gen(function* () {
          ...
          const source = yield* createDependentSource({
            name: `etherscan.${provider.chainId}.balance`,
            ...
            fetch: (_dep, forceRefresh) =>
              provider.fetchBalance(address, forceRefresh).pipe(
                Effect.map((raw): BalanceOutput => {
                  ...
                  return {
                    amount: Amount.fromRaw(raw.result, decimals, symbol),
                    symbol,
                  }
                })
              ),
...
export function createEtherscanV1ProviderEffect(entry: ParsedApiEntry, chainId: string): ApiProvider | null {
  if (entry.type === "etherscan-v1" || entry.type.includes("blockscout")) {
    return new EtherscanV1ProviderEffect(entry, chainId)
  }
  return null
}
