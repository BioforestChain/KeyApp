...
function logApiFailure(name: string, payload: { success: boolean; error?: unknown }): void {
  ...
  if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
    console.warn(`[ethwallet] ${name} success=false`, payload.error ?? payload)
  }
}
...
function assertHistoryHealthy(
  ...
  if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
    console.warn(`[ethwallet] ${name} NOTOK`, raw)
  }
  return Effect.fail(new HttpError(`[ethwallet] ${name} NOTOK`))
}
...
export class EthWalletProviderEffect extends EvmIdentityMixin(EvmTransactionMixin(EthWalletBase)) implements ApiProvider {
  ...
  constructor(entry: ParsedApiEntry, chainId: string) {
    ...
    this.tokenBalances = createStreamInstanceFromSource<AddressParams, TokenBalancesOutput>(
      `ethwallet.${chainId}.tokenBalances`,
  ...
  private createBalanceSource(
    ...
    return this.getSharedBalanceSource(params.address).pipe(
      Effect.catchAllCause((error) => {
        if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
          console.warn("[ethwallet] balance fallback", error)
        }
  ...
  private createTokenBalancesSource(
    ...
    return this.getSharedTokenBalanceSource(params.address).pipe(
      Effect.catchAllCause((error) => {
        if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
          console.warn("[ethwallet] tokenBalances fallback", error)
        }
  ...
  private createBalanceFallbackSource(address: string): Effect.Effect<DataSource<BalanceOutput>> {
    ...
    return createPollingSource({
      name: `ethwallet.${this.chainId}.balance.fallback`,
  ...
  private createTokenBalancesFallbackSource(address: string): Effect.Effect<DataSource<TokenBalancesOutput>> {
    ...
    const decimals = this.decimals
    return createPollingSource({
      name: `ethwallet.${this.chainId}.tokenBalances.fallback`,
      interval: Duration.millis(this.pollingInterval),
      immediate: true,
      fetch: this.fetchTokenBalances(address, [], true).pipe(
        Effect.map((balances): TokenBalancesOutput => {
          ...
          for (const item of balances.tokens) {
            const tokenSymbol = item.symbol ?? "ERC20"
      ...
      const createPromise = Effect.runPromise(
        Effect.gen(function* () {
          ...
          const nativeSource = yield* createPollingSource({
            name: `ethwallet.${provider.chainId}.txHistory.native.${cacheKey}`,
          ...
          if (!isHistoryHealthy(initialNative)) {
            ...
            return yield* Effect.fail(new HttpError("[ethwallet] trans/normal/history NOTOK"))
          }
          ...
          const attachContract = (contract: string) =>
            Effect.gen(function* () {
              ...
              const source = yield* createPollingSource({
                name: `ethwallet.${provider.chainId}.txHistory.erc20.${cacheKey}.${contract}`,
          ...
          const mergeSource = yield* createDependentSource({
            name: `ethwallet.${provider.chainId}.txHistory.${cacheKey}`,
        ...
        Effect.gen(function* () {
          const txHistorySource = yield* Effect.catchAll(
            ...
            (error) =>
              Effect.gen(function* () {
                if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
                  console.warn("[ethwallet] txHistory unavailable for balance", error)
                }
                return null
              })
          )
          ...
          if (txHistorySource === null) {
            const source = yield* createPollingSource({
              name: `ethwallet.${provider.chainId}.balance.poll`,
              interval: Duration.millis(provider.pollingInterval),
              immediate: true,
          ...
          const dependentSource = yield* createDependentSource({
            name: `ethwallet.${provider.chainId}.balance.dep`,
          ...
          const mergeSource = yield* createDependentSource({
            ...
            fetch: () =>
              Effect.gen(function* () {
                const cached = yield* SubscriptionRef.get(balanceCache)
                if (!cached) {
                  return yield* Effect.fail(new HttpError("[ethwallet] balance cache empty"))
                }
                return cached
              }),
          })
        ...
        Effect.gen(function* () {
          const txHistorySource = yield* Effect.catchAll(
            ...
            (error) =>
              Effect.gen(function* () {
                if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
                  console.warn("[ethwallet] txHistory unavailable for tokenBalances", error)
                }
                return null
              })
          )
          ...
          if (txHistorySource === null) {
            const source = yield* createPollingSource({
              name: `ethwallet.${provider.chainId}.tokenBalances.poll`,
              interval: Duration.millis(provider.pollingInterval),
              immediate: true,
          ...
          const dependentSource = yield* createDependentSource({
            name: `ethwallet.${provider.chainId}.tokenBalances.dep`,
            dependsOn: txHistorySource.ref,
            hasChanged: hasTransactionListChanged,
          ...
          const mergeSource = yield* createDependentSource({
            name: `ethwallet.${provider.chainId}.tokenBalances`,
            dependsOn: mergeSignal,
            hasChanged: (prev, next) => prev !== next,
            fetch: () =>
              Effect.gen(function* () {
                const cached = yield* SubscriptionRef.get(balanceCache)
                if (!cached) {
                  return yield* Effect.fail(new HttpError("[ethwallet] tokenBalances cache empty"))
                }
                return cached
              }),
          })
  ...
  private getSharedTokenListSource(): Effect.Effect<DataSource<TokenListResponse>> {
    ...
    return Effect.promise(async () => {
      const createPromise = Effect.runPromise(
        Effect.gen(function* () {
          const source = yield* createPollingSource({
            name: `ethwallet.${provider.chainId}.tokenList`,
  ...
  private fetchNativeHistory(
    ...
    return httpFetchCached({
      ...
      Effect.flatMap((raw) => assertHistoryHealthy("trans/normal/history", raw))
    )
  }
  ...
  private fetchTokenHistory(
    ...
    return httpFetchCached({
      url: `${this.baseUrl}/trans/erc20/history`,
      method: "POST",
      body: {
        address: params.address,
      ...
      Effect.tap((raw) => logApiFailure("trans/erc20/history", raw)),
      Effect.flatMap((raw) => assertTokenHistoryHealthy("trans/erc20/history", raw))
    )
  }
