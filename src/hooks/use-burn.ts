...
function validateAmountInput(amount: Amount | null, asset: AssetInfo | null): string | null {
  ...
  if (!amount.isPositive()) {
    return t('error:validation.enterValidAmount');
  }

  const balance = asset.amount;
  if (amount.gt(balance)) {
    return t('error:validation.exceedsBalance');
  }
...
export function useBurn(options: UseBurnOptions = {}): UseBurnReturn {
  const { initialAsset, assetLocked = false, useMock = true, walletId, fromAddress, chainConfig } = options;
        ...
        try {
          ...
          if (!applyAddress) {
            setState((prev) => ({
              ...prev,
              feeLoading: false,
              errorMessage: t('error:transaction.issuerAddressNotFound'),
            }));
            return;
          }
  ...
  const canProceed = useMemo(() => {
    return !!(state.asset && state.amount && state.amount.isPositive() && state.recipientAddress && !state.feeLoading);
  ...
  const goToConfirm = useCallback((): boolean => {
    ...
    if (!state.recipientAddress) {
      setState((prev) => ({
        ...prev,
        errorMessage: t('error:transaction.issuerAddressNotReady'),
      }));
      return false;
    }
  ...
  const submit = useCallback(
    async (password: string): Promise<BurnSubmitResult> => {
      if (useMock) {
        setState((prev) => ({
          ...prev,
          step: 'burning',
