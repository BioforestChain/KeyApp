...
describe('AmountInput', () => {
  ...
  it('shows MAX button when balance is provided', () => {
    render(<AmountInput balance="1000" />)
    expect(screen.getByRole('button', { name: 'MAX' })).toBeInTheDocument()
  })

  it('sets max value when MAX button clicked', async () => {
    const handleChange = vi.fn()
    render(<AmountInput balance="1000" max="1000" onChange={handleChange} />)
  ...
  it('shows fiat value when provided', () => {
    render(<AmountInput value="100" fiatValue="100.00" />)
    expect(screen.getByText(/≈ \$100.00/)).toBeInTheDocument()
  })

  it('shows custom fiat symbol', () => {
    render(<AmountInput value="100" fiatValue="800" fiatSymbol="¥" />)
    expect(screen.getByText(/≈ ¥800/)).toBeInTheDocument()
  })

  it('shows error when value exceeds max', () => {
    render(<AmountInput value="5000" max="1000" />)
    expect(screen.getByText('超出可用余额')).toBeInTheDocument()
  })

  it('shows custom error message', () => {
    render(<AmountInput error="最小金额为 10" />)
  ...
  it('hides MAX button when disabled', () => {
    render(<AmountInput balance="1000" disabled />)
