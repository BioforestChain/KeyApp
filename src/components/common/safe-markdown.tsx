...
function toInlineNodes(text: string): Array<string | JSX.Element> {
...
function toBoldNodes(text: string, keyPrefix = 'b'): Array<string | JSX.Element> {
  const parts = text.split('**')
  ...
  return (
    <div className={cn('space-y-3 text-sm', className)} data-testid="safe-markdown">
      {blocks.map((block, index) => {
        ...
        if (block.type === 'ul' || block.type === 'ol') {
          ...
          return (
            <ListTag
              key={`l:${index}`}
              className={cn(
                'text-muted-foreground space-y-1 ps-5',
        ...
        return (
          <p key={`p:${index}`} className="text-muted-foreground whitespace-pre-wrap leading-relaxed">
            {toInlineNodes(block.text).map((node, i) => (
