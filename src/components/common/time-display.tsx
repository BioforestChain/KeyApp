...
export function TimeDisplay({ value, format = 'relative', className }: TimeDisplayProps) {
...
function formatRelativeTime(date: Date, t: TFunction, locale: string): string {
  ...
  if (absDiff < 3600000) {
    ...
    return isFuture ? t('time.minutesLater', { count }) : t('time.minutesAgo', { count });
  }

  if (absDiff < 86400000) {
    const count = Math.floor(absDiff / 3600000);
    return isFuture ? t('time.hoursLater', { count }) : t('time.hoursAgo', { count });
  }

  if (absDiff < 604800000) {
    const count = Math.floor(absDiff / 86400000);
    return isFuture ? t('time.daysLater', { count }) : t('time.daysAgo', { count });
  }
