...
function RecordItem({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-lg bg-card p-3 text-left transition-colors hover:bg-muted/50"
    >
      {/* Type icon */}
      <div
        className={cn(
      ...
      >
        <TypeIcon
          className={cn('size-5', isMint ? 'text-green-500' : 'text-orange-500')}
        />
      </div>
...
export function StakingRecordList({
  filterType = 'all',
  onRecordClick,
  className,
}: StakingRecordListProps) {
  const { t } = useTranslation('staking')
