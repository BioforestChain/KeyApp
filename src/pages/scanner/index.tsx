      ...
      <div className="relative flex-1">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          <div className="relative size-64">
            <div className={cn(
              'absolute left-0 top-0 size-8 border-l-4 border-t-4',
              state === 'success' ? 'border-green-500' : 'border-primary'
            )} />
            <div className={cn(
              'absolute right-0 top-0 size-8 border-r-4 border-t-4',
              state === 'success' ? 'border-green-500' : 'border-primary'
            )} />
            <div className={cn(
            ...
            <div className={cn(
              'absolute bottom-0 right-0 size-8 border-b-4 border-r-4',
              state === 'success' ? 'border-green-500' : 'border-primary'
            )} />

            {/* Scan line animation */}
            {state === 'scanning' && (
              <div className="absolute left-2 right-2 h-0.5 animate-scan bg-primary" />
            )}
          </div>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-32 text-center">
          {state === 'idle' && (
            <p className="text-white/80">{t('initializing')}</p>
          )}
          {state === 'requesting' && (
            <p className="text-white/80">{t('requestingPermission')}</p>
          )}
          {state === 'scanning' && (
            <p className="text-white/80">{t('scanPrompt')}</p>
          )}
          {state === 'success' && (
