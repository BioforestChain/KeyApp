...
function NotificationItem({
  ...
  return (
    <div
      onClick={handleClick}
      className={cn(
        'flex items-start gap-3 rounded-xl p-4 transition-colors cursor-pointer',
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {!notification.read && (
          <div className="mt-2 flex size-2 rounded-full bg-primary" aria-label="未读" />
        )}
      </div>
  ...
  if (notifications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">您的所有通知都会显示在这里</p>
      </div>
    )
  }
        ...
        rightAction={
          state.unreadCount > 0 ? (
            <button
              ...
              className="flex items-center gap-1 rounded-full px-3 py-1.5 text-sm text-primary hover:bg-primary/10"
              aria-label="全部已读"
            >
      ...
      <div className="border-b bg-card px-4 py-3">
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            {state.unreadCount > 0 ? (
              <>
                <span className="font-medium text-foreground">{state.unreadCount}</span> 条未读
              </>
            ) : (
              '没有未读通知'
            )}
