...
test.describe('交易历史 - 金额显示测试', () => {
  test('交易列表显示格式化金额', async ({ page }) => {
    ...
    if (count > 0) {
      // 验证金额以格式化形式显示（带小数点和符号）
      const firstItem = transactionItems.first()
      await expect(firstItem.locator('text=/\\d+\\.?\\d*/')).toBeVisible()
    }
  })
  ...
  test('交易历史过滤器工作正常', async ({ page }) => {
    ...
    if (await chainFilter.isVisible()) {
      await chainFilter.selectOption({ index: 0 })
      await page.waitForTimeout(300)
    }

    if (await periodFilter.isVisible()) {
      await periodFilter.selectOption({ index: 0 })
      await page.waitForTimeout(300)
    }
  })
})
...
test.describe('交易历史 - 空状态测试', () => {
  test('无交易时显示空状态', async ({ page }) => {
    ...
    // 验证显示空状态消息
    const emptyState = page.locator('text=/no.*transaction|暂无|empty/i')
    // 空状态可能是预期的
  })
...
test.describe('交易历史 - 刷新功能测试', () => {
  test('下拉刷新或刷新按钮', async ({ page }) => {
    ...
    // 查找刷新按钮
    const refreshButton = page.locator('[aria-label*="刷新"], [aria-label*="refresh"], button:has-text("刷新")')
